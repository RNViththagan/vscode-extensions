...
export function createConnectorGeneratorTool(eventHandler?: CopilotEventHandler, projectName?: string) {
    return tool({
        description: `Generates Ballerina connector modules from OpenAPI specifications for unknown/custom services.

**When to use:**
- For services NOT in LibraryProviderTool (internal/custom APIs, third-party services without Ballerina support)
- When you need a type-safe client for an external API
- During service_design or HTTP client implementation tasks

**What it does:**
1. Requests OpenAPI spec from user (supports both JSON and YAML formats)
2. Generates complete Ballerina connector module with client class, typed methods, record types, and authentication configuration
3. Returns module information with generated file paths and their complete content

**Result:**
- moduleName: The generated module name
- importStatement: Import statement to use in your code
- generatedFiles: Array of objects with path and content for each generated .bal file`,
