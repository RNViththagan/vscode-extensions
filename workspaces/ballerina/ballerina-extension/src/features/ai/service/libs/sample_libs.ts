...
import { getFunctionsResponseSchema } from "./funcs_inter_types";
...
Treat the tool output as raw input — **your final response must contain only filtered, relevant information**.
  `.trim(),
    ...
    console.log("Generated library selection experimental output:", results.experimental_output);

    // Try experimental output first, with type guard and filter
    let filteredOutput = undefined;
    if (results.experimental_output && Array.isArray(results.experimental_output)) {
        filteredOutput = results.experimental_output.filter(
